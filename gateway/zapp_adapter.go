...
func newServiceAdapter(app core.IApp) core.IService {
	defServiceOnce.Do(func() {
		...
		if err != nil {
			log.Panic("grpc网关服务配置错误", zap.String("serviceType", string(DefaultServiceType)), zap.Error(err))
		}

		g, err := NewGateway(app, conf)
		if err != nil {
			log.Panic("创建grpc网关服务失败", zap.String("serviceType", string(DefaultServiceType)), zap.Error(err))
		}
		defService = &ServiceAdapter{
			app:    app,
			server: g,
